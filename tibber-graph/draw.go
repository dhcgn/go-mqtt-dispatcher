...
const (
	...
	THRESHOLD_ONE_PIXEL    = 0.3000
	THRESHOLD_TWO_PIXELS   = 0.5000
	THRESHOLD_THREE_PIXELS = 0.6000

	// Threshold for large price difference
	THRESHOLD_LARGE_DIFFERENCE = 0.8000
)
...
func calculateY(price, currentPrice float64) int {
	...
	y := 4 // Center (adjusted for 8x32 grid)
	...
	if absDiff < THRESHOLD_ONE_PIXEL {
		...
		if absDiff > THRESHOLD_LARGE_DIFFERENCE {
			return y // White pixel for large differences
		}
	}
...
// Helper function to print data in a human-readable format
